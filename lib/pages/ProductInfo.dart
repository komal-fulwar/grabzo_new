...
class _ProductInfoState extends State<ProductInfo> {
        ...
        SingleChildScrollView(
          ...
          child: Column(
            children: [
              ...
              FutureBuilder(
                  future: Items().getAllShopsItems(data.shopId),
                  builder:
                      (BuildContext context, AsyncSnapshot<ShopBean> snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      return Center(child: CircularProgressIndicator());
                    } else {
                      if (snapshot.hasError)
                        ...
                        return productColumn(snapshot.data);
                      }
                    }
                  }),
            ],
  ...
  Column productColumn(ShopBean data) {
    return Column(
      children: [
        // int itm =  6;
        for (int i = 0; i < data.items.length / 2; i++)
          Row(
            children: [
              //this
              for (int j = 0; j < 2; j++)
                (data.items.length <= i * 2 + j)
                    ? Text("")
                    : Padding(
